...
import {BufferChunk, ExtrudedMeshGeneratorOptions} from './ExtrudedMeshGenerator'
...
export class MeshPrototypeGenerator {
	...
	public addCurve(path: FiberCurve): BufferChunk {
		...
		for (let j = 0; j < LENGTH_SEGMENTS; ++j) {
			...
			let point = new THREE.Vector3(path[j].position[0], path[j].position[1], path[j].position[2])
			let radius = path[j].width / 2

			if (j > 0) {
				let prev = new THREE.Vector3(path[j - 1].position[0], path[j - 1].position[1], path[j - 1].position[2])
				dir1 = new THREE.Vector3().subVectors(point, prev)
			}

			if (j < LENGTH_SEGMENTS - 1) {
				let next = new THREE.Vector3(path[j + 1].position[0], path[j + 1].position[1], path[j + 1].position[2])
	...
	private getIndiciForLayer(index: number, layer: number, numPoints: number): number[] {
		const arr: number[] = []
		let a, b, c
